...
def load_from_snapshot(value,ptype,sdir,snum,particle_mask=numpy.zeros(0),axis_mask=numpy.zeros(0),
    ...
    if not ('NumFilesPerSnapshot' in header_toparse and 'NumPart_Total' in header_toparse
        ...
        and ('HubbleParam' in header_toparse or 'Hubble_Param' in header_toparse) and 'NumPart_ThisFile' in header_toparse):
    ...
    if(units_to_physical):
        ...
        hubble = 1;
        if('HubbleParam' in header_toparse): hubble = header_toparse["HubbleParam"]
        if('Hubble_Param' in header_toparse): hubble = header_toparse["Hubble_Param"]
        cosmological = False
        ascale = 1.0;
...
def check_if_filename_exists(sdir,snum,snapshot_name='snapshot',snapdir_name='snapdir',extension='.hdf5',
    ...
    for extension_touse in [extension,'.h5','.bin','']:
        ...
        if not os.path.exists(fname): 
            ...
            fname=fname_base+'.0'+extension_touse;
        if not os.path.exists(fname):
            ## is it in a snapshot sub-directory? (is it a single file?)
            fname_base=sdir+'/'+snapdir_name+'_'+ext+'/'+snapshot_name+'_'+ext;
            fname=fname_base+extension_touse;
        if not os.path.exists(fname): 
